# gwp-scratch — 30 保底轨（冻结）

> 版本：V1.0-frozen ｜ 更新：2026-09-12 ｜ 创建：2026-09-10
> 角色：**本目录已降级为 30.2 保底轨**（`master` 分支冻结）。它的验收底线只有三条：Plasma 里看得到图标、点得起来、socket `gwp30` 不与日用抢名。“31 出问题时还能用一个能用的编辑器”就是它的全部价值——不参与 31 线演进，不接收任何 merge。
> 区域宪法（三处载体、发布纪律、路径与回退）：[../AGENTS.md](../AGENTS.md) ｜ 31 线仓库宪法：[../emacs-dev/AGENTS.md](../emacs-dev/AGENTS.md) ｜ 区域经验库：[../docs/learnings.md](../docs/learnings.md)
>
> ⚠️ **本文件主体是该目录作为 31 线主仓时期（2026-09-10 至 09-12）的原文，保留作历史。**其中“加载拓扑”的三处载体表、“维护约定”里的 dev／部署路径、“验证”里的 profile 写法均已过时；本目录当前跑的是自己这份**冻结老配置**（无 `early-init.el`，包树与运行态就留在本目录里）。

## 职责三要素

- **做什么**：维护一套冻结的 Emacs 30 保底环境，验收底线只有三条——保底入口能启动、能进行基本编辑、socket `gwp30` 不与日用抢名。
- **不做什么**：不参与 31 活动线演化、不接收活动仓库的 merge、不按日常需求改造、不承担新配置的兼容性。
- **谁消费我**：31 日用或开发环境不可用时才进入本目录排障；日常维护不读本文件。
