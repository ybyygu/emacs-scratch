...
- 两类“像坏了其实别动”的情况：`$0`/`$1` 在少数提示词里是**有意**的光标落点（插入后光标落在 `<ybyygu>` 之类的位置）；`yas-indent-line` 会把展开文本按 markdown 规则重排缩进，嵌套子项可能变平级——只动行首空白、不动文字。真要逐字保缩进时，给该文件加 `# expand-env: ((yas-indent-line 'none))`（已验证可行）。
