...
- **2026-09-11 13:10 附件目录分窗**：查清“打开附件目录总落在下方”不是默认偏好，而是 `split-window-sensibly` 的阈值兜底（帧 182×42：上下要 ≥80 行、左右要 ≥200 列，两个方向都不满足，只能上下分）；新增 `gwp::org-open-file-other-window` 让目录走显式 `(split-window ... 'left)`（左窗满高、拿焦点），接线在 `org-link-frame-setup` 的 `file` 项，非目录文件与全局阈值都不动。待日用实感后再定两个旋钮：dired 是否定宽（SIZE 写 `-N`）、焦点是否留在 org。
